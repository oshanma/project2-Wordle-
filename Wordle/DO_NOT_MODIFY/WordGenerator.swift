...
class WordGenerator {
    static func generateGoalWord(with theme: WordTheme) -> [String] {
        guard let numLetters = SettingsManager.shared.settingsDictionary[kNumLettersKey] as? Int else {
            print("Error: numLetters is not set correctly")
            return []
        }

        guard let randomWord = theme.possibleWords(numLetters: numLetters).randomElement() else {
            print("Error: No words available for the given settings")
            return []
        }
